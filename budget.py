...
def filter_activities(activities, max_price, until=None, start=None):
    """Keep only activities we can stand behind: a real link, a real price within
    the budget, and a date (if any) that is today or later and inside the window.
    ...
    for item in activities:
        ...
        if when_date is not None:
            ...
            if when_date < date.today():
                continue
            if until is not None and when_date > until:
                continue
            if start is not None and when_date < start:
                continue
